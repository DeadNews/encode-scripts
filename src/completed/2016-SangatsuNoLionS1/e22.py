...
op = dn.oped(epis, name="op2", offset=36, start=OP, end=OPend, desc_h=desc_h)
ed = aaep.std.Trim(ED, EDend - 1)  # e22
...
mrgc = dn.rfs_resc(mrgc, epis, desc_h=desc_h + 1, mthr=30, maps=f"[34146 {EDend-1}]")  # e22
# ------------ #
...
F3 = dn.filt(mrgc, db_saveblack=0, db_thr=1.5, rt_sigma=0.6)

F1 = dn.rfs(F1, F2, f"[{OP} {OPend-1}] [{ED} {EDend-1}]")
F1 = dn.rfs(F1, F3, f"[{OP} {OPend-1}]")  # op2
# ------------ #
...
# dn.rfs_resc(mrgc, epis, desc_h=desc_h+1, mthr=30, out_mask=True).set_output()
# ------------ #
