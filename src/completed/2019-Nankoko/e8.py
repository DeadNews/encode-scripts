...
# ----mrgc---- #
aaep = dn.aa(epis)
op = dn.oped(epis, name="op3", offset=0, start=OP, end=OPend)
ed = dn.oped(epis, name="ed3", offset=585 - 94 - 1, start=ED, end=EDend)

mrgc = aaep.std.Trim(0, OP - 1) + op + aaep.std.Trim(OPend, ED - 1) + ed
# ------------ #

# ----mask---- #
maps = [(OPend, OPend + 130)] if PreED == ED else [(OPend, OPend + 130), (15458, ED - 1)]
